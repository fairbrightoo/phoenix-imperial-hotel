...
const NavigationSidebar = () => {
  ...
  return <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-64 bg-sidebar text-sidebar-foreground flex-col z-50 border-r border-sidebar-border">
    ...
    <nav className="flex-1 flex flex-col justify-center px-8 space-y-8">
      {links.map(link => <a key={link} href={`#${link.toLowerCase()}`} className="text-[#FEFCF9] hover:text-amber-400 transition-colors uppercase text-xl tracking-widest font-light flex items-center group">
        <span className="w-0 group-hover:w-4 transition-all duration-300 h-[1px] bg-amber-400 mr-0 group-hover:mr-3"></span>
        {link}
      </a>)}
    </nav>

    <div className="p-8 pt-4 border-t border-sidebar-border">
      <div className="flex items-center gap-3 text-amber-500 mb-2">
        <Phone size={18} />
        <span className="font-serif italic text-lg">+929-333-9296</span>
      </div>
      <p className="text-zinc-600 text-xs mt-4">© 2024 Phoenix Imperial Hotel</p>
    </div>
  </aside>;
};
const MobileNav = () => {
...
// @component: PhoenixImperialHotel
export const PhoenixImperialHotel = () => {
    ...
    <main className="md:ml-64 w-full md:w-[calc(100%-16rem)] min-h-screen relative">
      ...
      <section className="px-6 md:px-16 py-16 text-center max-w-7xl mx-auto">
        ...
        <p className="text-zinc-400 max-w-2xl mx-auto mb-16 leading-relaxed">
          Immerse yourself in a world of luxury and convenience. Our carefully curated facilities are designed to provide you with the ultimate comfort during your stay at Phoenix Imperial.
        </p>
      ...
      <footer className="bg-zinc-900 border-t border-zinc-800 pt-20 pb-10 px-6 md:px-16">
        ...
        <div className="max-w-7xl mx-auto border-t border-zinc-800 pt-8 flex flex-col md:flex-row justify-between items-center text-xs text-zinc-600 uppercase tracking-wider">
          <p>© 2024 Phoenix Imperial Hotel. All rights reserved.</p>
