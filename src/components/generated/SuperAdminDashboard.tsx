...
import { MOCK_BOOKINGS, ROOMS_BY_BRANCH } from './mockData';
...
export const SuperAdminDashboard: React.FC<SuperAdminDashboardProps> = ({
  ...
  const { branches, updateBranch, addBranch, globalContent, updateGlobalContent, systemSettings, updateSystemSettings } = useTenant();
  ...
  const handleSaveUser = (data: UserFormData) => {
    if (editingUser) {
      ...
      const newUser: User = {
        ...
        role: data.role,
        branchId: data.role === 'branch_admin' ? data.branchId : undefined
      };
      addUser(newUser);
    }
  ...
  return <div className="flex h-full bg-zinc-900 text-white relative">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-amber-400 font-serif">
                    ₦
                    {MOCK_BOOKINGS.filter(b => b.branchId === 'abuja' && (b.status === 'confirmed' || b.status === 'completed')).reduce((sum, b) => sum + b.totalPrice, 0).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-zinc-800 rounded-lg border border-zinc-700 p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-amber-400 font-serif">
                    ₦
                    {MOCK_BOOKINGS.filter(b => b.branchId === 'lagos' && (b.status === 'confirmed' || b.status === 'completed')).reduce((sum, b) => sum + b.totalPrice, 0).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-zinc-800 rounded-lg border border-zinc-700 overflow-hidden">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-zinc-700">
                {users.filter(u => u.role !== 'customer').map(admin => <tr key={admin.id} className="hover:bg-zinc-750">
